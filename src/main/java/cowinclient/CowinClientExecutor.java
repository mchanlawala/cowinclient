...
public class CowinClientExecutor implements Callable<String> {
	...
	public CowinClientExecutor(Properties prop, LocalDate date, int pincode) {
		this.prop = prop;
		this.date = date;
	...
	public String call() throws Exception {
		try {
			...
			ResponseEntity<?> response = restTemplate.exchange(
					prop.getProperty("cowin.client.url") + "?pincode=" + pincode + "&" + "date="
			...
			for (LinkedTreeMap<?, ?> center : (List<LinkedTreeMap<?, ?>>) responseTree.get("centers")) {
				...
				for (LinkedTreeMap<?, ?> session : (List<LinkedTreeMap<?, ?>>) center.get("sessions")) {
					...
					if (availableCapacity.intValue() > 0 && minAge.intValue() >= 18
						...
						mailCentersInfo = "<tr>" + "<td>" + center.get("name") + "</td>" + "<td>"
